...
Of course, the team wants to test-drive this method. The purpose of test-driving this very simple method is to ensure all the plumbing for Rspec is configured properly. That way, the team can move ahead with more significant work without running into issues with the unit testing framework. 

They create a file named spec/handler_spec.rb with the following contents: 
...
describe Handler do
  ...
  it "returns the name of the service" do 
    expect(handler.default['service'])
        .to eq('galaxy')
  end
  
  it "returns the description of the service" do 
    expect(handler.default['description'])
        .to eq('Merchant\'s Guide to the Galaxy implementation')
  end
end
``` 

After the usual Ruby routine - forgetting the _require_ statement - and the usual TDD routine - test fails because there's no code, etc. - they end up with this implementation:
